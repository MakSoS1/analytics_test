...
def main() -> int:
    ...
    selected = build_v3_causal_plan(
        selected, topology=topology, seed=args.seed, matched_fraction=args.v3_matched_fraction,
    )
    ...
    fixtures = args.out / "inert-fixtures"; fixtures.mkdir(exist_ok=True)
    ...
    summary = {
        "requested": args.count, "executed": len(executed), "status_counts": dict(status_counts),
        "protocol_counts": dict(protocol_counts), "label_counts": dict(label_counts),
        "implementation_counts": dict(implementation_counts), "family_counts": dict(family_counts),
        "source_identity_count": source_count, "source_identity_required": source_required,
        ...
        "train_protocols": list(legacy.TRAIN_PROTOCOLS), "planner": "digital_twin_v3_causal",
        "causal_pairing": "full_provisional_replay_stable",
        "label_assignment_policy": "observable_baseline_or_mutation_then_label",
        "wire_controls_label_dependent": False, "implementation_choice_label_dependent": False,
        "external_targets_allowed": False, "payload_execution_allowed": False,
        "v3_signal": signal, "v3_campaigns": audit_v3_campaigns(selected), "failures": failures[:20],
    }
    (args.out / "summary.json").write_text(json.dumps(summary, indent=2, sort_keys=True) + "\n")
    print(json.dumps(summary, sort_keys=True))
    if failures or status_counts.get("success", 0) != args.count:
        return 1
