...
STATE_DIR="$OUT_ROOT/state/$SHARD"; RUN_DIR="$OUT_ROOT/work/$SHARD"; RELEASE="$OUT_ROOT/release"; BRONZE="$RELEASE/bronze/$SHARD"; QUALITY="$RELEASE/quality/$SHARD"; RAW_PCAP="$RUN_DIR/$SHARD.pcap"; BRONZE_PCAP="$BRONZE/captures/$SHARD.pcap"; TCPDUMP_PGID=""
...
cp --reflink=auto "$RAW_PCAP" "$BRONZE_PCAP"
[[ -s "$BRONZE_PCAP" ]]; tcpdump -nn -r "$BRONZE_PCAP" -c 1 >/dev/null 2>&1
...
PCAP_BYTES="$(stat -c%s "$BRONZE_PCAP")"; SESSION_COUNT="$(wc -l < "$BRONZE/manifests/sessions.jsonl")"; SUMMARY="$RUN_DIR/scenarios/summary.json"
"$PYTHON_BIN" - "$SUMMARY" "$QUALITY/capture_health.json" "$SHARD" "$STAGE" "$SESSION_COUNT" "$PCAP_BYTES" <<'PY'
import json,sys
...
p={
 ...
 'pcap_bytes':int(sys.argv[6]),
 'capture_format':'temporary-merged-pcap','capture_compression':'none','final_merged_capture_retained':False,
...
echo "v3_causal_temporary_bronze_ready=$BRONZE sessions=$SESSION_COUNT pcap_bytes=$PCAP_BYTES"
