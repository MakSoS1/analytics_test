...
cleanup() {
  set +e
  stop_capture
  "$PYTHON_BIN" "$ROOT/scripts/start_extended_services.py" stop "$STATE_DIR/extended"
  bash "$ROOT/scripts/start_services.sh" stop "$STATE_DIR/core"
  bash "$ROOT/scripts/setup_topology.sh" down "$ROOT/configs/topology.yaml"
}
...
PYTHONPATH="$ROOT/src" "$PYTHON_BIN" "$ROOT/scripts/run_scenarios_extended.py" \
  --stage "$STAGE" \
  --count "$COUNT" \
  --seed "$SEED" \
  --core-state "$STATE_DIR/core" \
...
[[ -s "$RAW_PCAP" ]]
tcpdump -nn -r "$RAW_PCAP" -c 1 >/dev/null 2>&1
...
SUMMARY="$RUN_DIR/scenarios/summary.json"
python3 - "$SUMMARY" "$QUALITY/capture_health.json" "$SHARD" "$STAGE" "$SESSION_COUNT" "$RAW_BYTES" "$PCAP_BYTES" "$OUTPUT_UID" "$OUTPUT_GID" <<'PY'
import json,sys
summary=json.load(open(sys.argv[1],encoding='utf-8'))
payload={
  'ok': summary.get('status_counts',{}).get('failed',0)==0,
  ...
  'capture_format':'pcap.zst', 'full_capture_retained':True,
  'output_owner_uid':int(sys.argv[8]), 'output_owner_gid':int(sys.argv[9]),
  'protocol_counts':summary.get('protocol_counts',{}),
  'label_counts':summary.get('label_counts',{}),
  'dcerpc_train_included':False,
  'partial_winrm_included':summary.get('partial_winrm_included',False),
}
open(sys.argv[2],'w',encoding='utf-8').write(json.dumps(payload,indent=2,sort_keys=True)+'\n')
if not payload['ok']: raise SystemExit('scenario failures present')
PY
...
echo "extended_bronze_ready=$BRONZE sessions=$SESSION_COUNT pcap_zst_bytes=$PCAP_BYTES"
