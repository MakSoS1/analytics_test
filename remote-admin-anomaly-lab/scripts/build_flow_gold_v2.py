...
def main()->int:
    ...
    mapped=attach_behavior_time(mapped,sessions);splits,split_report=assign_grouped_splits(sessions,seed=args.split_seed);split_map=splits.set_index('session_id')['split'];mapped['split']=mapped['session_id'].map(split_map)
    if mapped['split'].isna().any():raise SystemExit('mapped flow missing split before state computation')
    features=build_split_isolated_production_flow_features(mapped);label_cols=['session_id','campaign_id','scenario_id','pair_id','label_binary','label_family','protocol','semantic_fidelity','src_host_id','dst_host_id','persona_id','task_id','campaign_type','historical_relation'];label_source=mapped[['uid','session_id']].rename(columns={'uid':'flow_uid'}).merge(sessions[[c for c in label_cols if c in sessions]],on='session_id',how='left',validate='many_to_one');label_source['split']=label_source['session_id'].map(split_map);aligned=features[['flow_uid','session_id']].merge(label_source,on=['flow_uid','session_id'],how='left',validate='one_to_one')
