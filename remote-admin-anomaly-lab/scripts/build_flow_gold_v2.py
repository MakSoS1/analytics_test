...
def main()->int:
    ...
    release=args.release.resolve();bronze=release/'bronze'/args.shard;silver=release/'silver'/args.shard;gold=release/'gold'/args.shard;quality=release/'quality'/args.shard;contract=load_yaml(args.feature_contract);validate_feature_contract(contract);sessions=pd.read_parquet(bronze/'manifests/sessions.parquet');conn=read_zstd_json_lines(silver/'zeek/conn.log.zst')
    ...
    splits,split_report=assign_grouped_splits(sessions,seed=args.split_seed);split_map=splits.set_index('session_id')['split'];mapped=mapped.copy();mapped['split']=mapped['session_id'].map(split_map)
    if mapped['split'].isna().any():raise SystemExit('mapped flow missing split before state computation')
    features=build_split_isolated_production_flow_features(mapped)
    label_cols=['session_id','campaign_id','scenario_id','pair_id','label_binary','label_family','protocol','semantic_fidelity','src_host_id','dst_host_id','persona_id','task_id','campaign_type','historical_relation']
    ...
    summary={'rows':len(matrix),'feature_count':len(selected.columns),'session_mapping_coverage':report['session_mapping_coverage'],'conn_mapping_coverage':report['conn_mapping_coverage'],'uid_alignment_coverage':len(aligned)/len(features),'leakage_ok':True,'state_partition_policy':'fresh_state_per_split','production_unit':'parser_flow','label_join_keys':['flow_uid','session_id'],'orchestrator_used_only_for':'labels_grouped_splits_and_simulated_time_mapping'};write(quality/'production_flow_gold.json',summary);print(json.dumps(summary,sort_keys=True));return 0
