...
def load_optional_ssh(silver:Path)->pd.DataFrame|None:
    ...
    return read_zstd_json_lines(path) if path.exists() and path.stat().st_size>0 else None
def main()->int:
    ap=argparse.ArgumentParser();ap.add_argument('--release',type=Path,required=True);ap.add_argument('--shard',required=True);ap.add_argument('--feature-contract',type=Path,default=ROOT/'configs/feature_contract.yaml');ap.add_argument('--split-seed',type=int,default=20260814);args=ap.parse_args();release=args.release.resolve();bronze=release/'bronze'/args.shard;silver=release/'silver'/args.shard;gold=release/'gold'/args.shard;quality=release/'quality'/args.shard;contract=load_yaml(args.feature_contract);validate_feature_contract(contract);sessions=pd.read_parquet(bronze/'manifests/sessions.parquet');conn=read_zstd_json_lines(silver/'zeek/conn.log.zst');conn=enrich_ssh_auth_by_uid(conn,load_optional_ssh(silver));mapped,report=map_zeek_flows_to_sessions(sessions,conn);write(quality/'production_flow_mapping.json',report)
