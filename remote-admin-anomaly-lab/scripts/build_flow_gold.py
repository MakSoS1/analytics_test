...
def main()->int:
    ...
    features=build_production_flow_features(mapped);splits,split_report=assign_grouped_splits(sessions,seed=args.split_seed)
    split_map=splits.set_index('session_id')['split'];labels_cols=['session_id','campaign_id','scenario_id','pair_id','label_binary','label_family','protocol','semantic_fidelity','src_host_id','dst_host_id']
    labels=mapped[['uid','session_id']].rename(columns={'uid':'flow_uid'}).merge(sessions[[c for c in labels_cols if c in sessions]],on='session_id',how='left',validate='many_to_one');labels['split']=labels['session_id'].map(split_map)
    if labels[['label_binary','split']].isna().any().any():raise SystemExit('flow labels/splits missing')
    selected=select_model_columns(features,contract);matrix=selected.copy();matrix['label_binary']=labels['label_binary'].astype(int).to_numpy();matrix['split']=labels['split'].astype(str).to_numpy()
    ...
    features.to_parquet(gold/'production_flow_features.parquet',index=False);labels.to_parquet(gold/'production_flow_labels.parquet',index=False);matrix.to_parquet(gold/'production_model_matrix.parquet',index=False)
    summary={'rows':len(matrix),'feature_count':len(selected.columns),'session_mapping_coverage':report['session_mapping_coverage'],'conn_mapping_coverage':report['conn_mapping_coverage'],'leakage_ok':True,'production_unit':'parser_flow','orchestrator_used_only_for':'labels_and_grouped_splits'};write(quality/'production_flow_gold.json',summary);print(json.dumps(summary,sort_keys=True));return 0
