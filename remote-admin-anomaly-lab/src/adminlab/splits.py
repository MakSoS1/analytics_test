...
def assign_grouped_splits(sessions: pd.DataFrame, *, seed: int = 20260814) -> tuple[pd.DataFrame, dict[str, Any]]:
    ...
    heldout_users = _choose_holdout(frame["src_host_id"].astype(str).tolist(), seed)
    ...
    group_forced_challenge: set[str] = set(temporal_groups)
    for gid, part in frame.groupby("group_id", sort=False):
        if set(part["src_host_id"].astype(str)) & heldout_users:
            group_forced_challenge.add(str(gid))
        if set(part["host_pair"].astype(str)) & heldout_pairs:
            group_forced_challenge.add(str(gid))

    assignments: dict[str, str] = {}
    for gid in sorted(frame["group_id"].astype(str).unique()):
        if gid in group_forced_challenge:
    ...
    report = {
        "seed": seed,
        "heldout_src_hosts": sorted(heldout_users),
...
def audit_leakage(
    ...
    merged = sessions[["session_id", "campaign_id", "pair_id", "src_host_id", "dst_host_id"]].merge(
    ...
    heldout_users = set(map(str, (split_report or {}).get("heldout_src_hosts", [])))
    ...
    heldout_pairs = set(map(str, (split_report or {}).get("heldout_host_pairs", [])))
    ...
    return {
        ...
        "pairs_crossing_splits": int(len(pair_leaks)) if not nonempty_pairs.empty else 0,
