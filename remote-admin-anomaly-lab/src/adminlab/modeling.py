...
def choose_threshold(y: np.ndarray, score: np.ndarray, *, max_fpr: float = 0.05) -> float:
...
def _split_xy(frame: pd.DataFrame, split: str) -> tuple[pd.DataFrame, np.ndarray]:
    ...
    y = subset.pop(LABEL).astype(int).to_numpy()
    subset.pop(SPLIT)
    return subset, y
...
def train_supervised(frame: pd.DataFrame, *, seed: int = 20260814) -> tuple[Pipeline, dict[str, Any]]:
    x_train, y_train = _split_xy(frame, "train")
    x_val, y_val = _split_xy(frame, "validation")
    if len(np.unique(y_train)) < 2:
        raise ValueError("supervised train split needs both classes")
    pipeline = build_supervised_pipeline(x_train, seed=seed)
    pipeline.fit(x_train, y_train)
    val_score = pipeline.predict_proba(x_val)[:, 1]
    threshold = choose_threshold(y_val, val_score, max_fpr=0.05)
    report: dict[str, Any] = {
        "model": "LightGBM",
        "threshold_policy": "max validation F1 subject to FPR<=0.05",
        "threshold": threshold,
        "splits": {"validation": _metrics(y_val, val_score, threshold)},
    }
    for split in ("test", "challenge"):
        try:
            x, y = _split_xy(frame, split)
        except ValueError:
            continue
        score = pipeline.predict_proba(x)[:, 1]
        report["splits"][split] = _metrics(y, score, threshold)
...
class BenignOnlyModel:
    ...
    def score(self, frame: pd.DataFrame) -> np.ndarray:
        x = frame[self.columns].apply(pd.to_numeric, errors="coerce").fillna(0.0).to_numpy(dtype=float)
        transformed = self.scaler.transform(x)
        # Higher means more anomalous/suspicious.
...
def train_benign_only(frame: pd.DataFrame, *, seed: int = 20260814) -> tuple[BenignOnlyModel, dict[str, Any]]:
    numeric_cols = [
        ...
        if c not in {LABEL, SPLIT} and pd.api.types.is_numeric_dtype(frame[c])
    ]
    if not numeric_cols:
        raise ValueError("no numeric production features for benign-only model")
    train = frame[(frame[SPLIT] == "train") & (frame[LABEL] == 0)]
    if len(train) < 20:
        raise ValueError("benign-only model needs at least 20 benign train rows")
    x_train = train[numeric_cols].fillna(0.0).to_numpy(dtype=float)
    ...
    if val.empty or val[LABEL].nunique() < 2:
        raise ValueError("benign-only validation requires both classes")
    temp = BenignOnlyModel(numeric_cols, scaler, model, threshold=0.0)
    val_score = temp.score(val)
    # Isolation scores are unbounded; threshold selection works directly.
    threshold = choose_threshold(val[LABEL].astype(int).to_numpy(), val_score, max_fpr=0.05)
    temp.threshold = threshold
    report: dict[str, Any] = {
        "model": "IsolationForest-benign-only",
        "features": numeric_cols,
        "threshold": threshold,
        "splits": {
            "validation": _metrics(val[LABEL].astype(int).to_numpy(), val_score, threshold)
        },
    }
    for split in ("test", "challenge"):
        ...
        if subset.empty:
            continue
        report["splits"][split] = _metrics(
            subset[LABEL].astype(int).to_numpy(), temp.score(subset), threshold
        )
    return temp, report
...
def evaluate_deterministic(frame: pd.DataFrame) -> dict[str, Any]:
    ...
    threshold = choose_threshold(
        val[LABEL].astype(int).to_numpy(), deterministic_score(val), max_fpr=0.05
    )
    report: dict[str, Any] = {
        "model": "deterministic-behavior-baseline",
        "threshold": threshold,
        "splits": {},
    }
    for split in ("validation", "test", "challenge"):
        ...
        if subset.empty:
            continue
        report["splits"][split] = _metrics(
            subset[LABEL].astype(int).to_numpy(), deterministic_score(subset), threshold
        )
    return report
