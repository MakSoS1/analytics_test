...
def campaign_relative_path(rows: list[SessionRecord]) -> Path:
    ...
    campaign_id = _safe_component(rows[0].campaign_id)
    return Path("campaigns") / label_name / f"{campaign_id}.pcap.zst"


def _capture_time(value: datetime) -> str:
    # editcap accepts this unambiguous UTC form.
...
def _session_evidence(row: SessionRecord, relative: Path, final_path: Path, packet_count: int) -> PcapEvidence:
    return PcapEvidence(
        ...
        semantic_family=str(row.campaign_type or row.label_family),
        session_id=str(row.session_id),
        campaign_id=str(row.campaign_id),
        start_ts=str(row.start_ts),
        src_host_id=str(row.src_host_id),
        dst_host_id=str(row.dst_host_id),
        implementation_id=str(row.implementation_id),
        semantic_fidelity=str(row.semantic_fidelity),
        packet_count=int(packet_count),
        pcap_bytes=int(final_path.stat().st_size),
        sha256=_sha256(final_path),
    )


def slice_session_pcaps(
    merged_pcap: Path,
    ...
    *,
    ...
    """Slice successful executed sessions from a temporary merged capture.

    The merged input is never copied into ``output_root``. Only verified compressed
    session PCAPs become authoritative Bronze evidence.
    """
    ...
    with tempfile.TemporaryDirectory(prefix="v3-session-pcap-") as tmp_dir:
        ...
        for row in sessions:
            ...
            if not final_path.is_file() or final_path.stat().st_size <= 0:
                raise ValueError(f"failed to persist session PCAP for {row.session_id}")
            evidence.append(_session_evidence(row, relative, final_path, packets))
    return evidence


def build_campaign_pcaps(
    session_evidence: Iterable[PcapEvidence],
    sessions: Iterable[SessionRecord],
    output_root: Path,
) -> list[PcapEvidence]:
    output_root = Path(output_root)
        ...
        for campaign_id, rows in sorted(rows_by_campaign.items()):
            ...
            output.append(
                PcapEvidence(
                    ...
                    start_ts=min(row.start_ts for row in rows),
...
def build_pcap_index(evidence: Iterable[PcapEvidence], sessions: Iterable[SessionRecord]) -> pd.DataFrame:
    # Keep context metadata for human inspection only. Model feature contracts
    # explicitly forbid these identity/semantic fields.
    rows = [item.__dict__.copy() for item in evidence]
    columns = [
        "kind", "label_binary", "label_name", "protocol", "semantic_family", "session_id",
        "campaign_id", "start_ts", "src_host_id", "dst_host_id", "implementation_id",
        "semantic_fidelity", "relative_pcap_path", "packet_count", "pcap_bytes", "sha256",
    ]
