...
def _row(
    ...
    *,
    ...
    return SessionRecord(
        campaign_id=f"campaign-{src}-{label}", scenario_id=f"causal-{family}", session_id=session_id,
        pair_id=pair_id, label_binary=label, label_family=family, mitre_technique="T1021.004",
        src_role="AdminWorkstation" if label == 0 else "Workstation", dst_role="Server",
        src_host_id=src, dst_host_id=dst, src_ip="10.77.0.10", dst_ip="10.77.0.200", src_port=0,
        dst_port={"ssh": 22, "smb": 445, "rdp": 3389, "vnc": 5900}[protocol], protocol=protocol,
        action="bounded_admin_session", wire_fidelity="real_wire", semantic_fidelity="high",
        ground_truth_source="scenario_orchestrator", netem_profile="normal", generator_seed=20260816,
        start_ts=start.isoformat(), end_ts=(start + timedelta(seconds=90)).isoformat(), status="planned",
        persona_id=f"persona-{src}", task_id="diagnostics", calendar_id="business_hours",
        intent_profile="approved" if label == 0 else "lateral_movement", behavior_profile="short_interactive",
        campaign_type=family, historical_relation="known_pair" if label == 0 else "novel_pair",
        auth_outcome="success", client_stack="openssh", server_stack="openssh-server",
        implementation_id=f"{protocol}:client->server", simulated_day=9, wire_attempts=1, wire_transfer_bytes=0,
    )
...
def test_counterfactual_pair_must_match_current_session_but_differ_in_causal_history():
    ...
    benign_history = [_row(f"b-{i}", label=0, src="src-a", dst="dst-known", minute=i + 1) for i in range(4)]
    suspicious_history = [_row(f"s-{i}", label=0, src="src-b", dst="dst-other", minute=i + 1) for i in range(4)]
...
def test_causal_observability_accepts_rare_pair_with_real_prior_difference():
    ...
    suspicious = _row("suspicious", label=1, src="src-b", dst="dst-new", pair_id=pair_id, family="rare_pair")
    history = {
        "benign": [_row("b1", label=0, src="src-a", dst="dst-known", minute=1), _row("b2", label=0, src="src-a", dst="dst-known", minute=2)],
        "suspicious": [_row("s1", label=0, src="src-b", dst="dst-other", minute=1), _row("s2", label=0, src="src-b", dst="dst-other", minute=2)],
    }
    report = audit_causal_observability([benign, suspicious], history_by_session=history)
...
def test_causal_planner_builds_balanced_observable_signal_before_labels():
    ...
    for index in range(80):
        ...
        row = _row(f"plan-{index:03d}", label=index % 2, src="placeholder-src", dst="placeholder-dst", protocol=protocol, minute=1 + (index % 50))
        start = datetime(2026, 6, 1, 8, 0, tzinfo=timezone.utc) + timedelta(hours=index * 4)
        rows.append(SessionRecord.from_dict({
            **row.to_dict(), "start_ts": start.isoformat(), "end_ts": (start + timedelta(seconds=90)).isoformat(),
    ...
    assert len({row.campaign_type for row in prepared if row.label_binary == 1}) >= 4
