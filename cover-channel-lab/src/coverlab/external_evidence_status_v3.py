...
def main():
    ...
    summary={'framework_ready':fw.get('validated',False),'framework_model_metrics_ready':fw.get('model_evaluation_ready',False),'wire_real_ech_ready':(ech.get('external_wire_real') or {}).get('validated',False),'ech_model_metrics_ready':(ech.get('external_wire_real') or {}).get('model_evaluation_ready',False),'environment_ready':bool(env.get('validated')) and bool(env.get('client_diversity_ready')) and bool(env.get('server_diversity_ready')) and bool(env.get('network_diversity_ready')) and bool(env.get('model_evaluation_ready')),'external_long_timing_ready':long.get('validated',False),'office_background_ready':office.get('validated',False)}
