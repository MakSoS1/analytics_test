...
def _append_config(manifest: Path, campaign_id: str, updates: dict):
    ...
    for key, value in updates.items():
        # Real protocol dispatchers own the factual client implementation.
        # Preserve the experiment's planned client choice separately rather
        # than relabelling aioquic/grpcio/paho traffic as a generic client.
        if key == "client_impl" and rec.get("generator_name") == "coverlab_protocol_dispatch":
            rec["configured_client_impl"] = value
        else:
            rec[key] = value
...
def invoke(
    ...
    ns = SimpleNamespace(
        ...
        variant="suspicious" if suspicious else "benign",
        seed=seed,
        ...
        source_ip=source_ip,
        events=events,
        client_impl=config.get("client_impl", "python_httpx"),
...
def parser_stage(args, manifest, events_out):
    # Source-plan Stage A: 60 suspicious + 60 paired benign, five repetitions.
    ...
    for s in scenarios:
        for rep in range(5):
            for suspicious in (True, False):
                ...
                config = {
                    "experiment_stage": "A_parser",
                    "configuration_id": f"A-{s.scenario_id}",
                    "transform_chain": [TRANSFORMS[rep % len(TRANSFORMS)]],
                    "timing_profile": TIMINGS[rep % 4],
                    "client_impl": "python_httpx",
                    "payload_size_class": SIZES[rep % 4],
                }
...
def isolated_stage(args, manifest, events_out):
    # 216 configs * (20 suspicious + 40 paired benign) = 12,960 sessions.
    core = select("core")
    ...
    for cfg in config_ids:
        ...
        for k in range(60):
            ...
            config = {
                "experiment_stage": "B_isolated",
                "configuration_id": f"B-{cfg:03d}",
                "transform_chain": [transform],
                "timing_profile": timing,
                "client_impl": client,
                "payload_size_class": size,
                "pair_group": f"B-{cfg:03d}-{rep % 20:02d}",
            }
...
def sequence_stage(args, manifest, events_out):
    # 72 profiles * 10 runs = 720 campaigns; 60 transactions each = 43,200.
    ...
    for p in profiles:
        for rep in range(10):
            ...
            config = {
                "experiment_stage": "C_sequence",
                "configuration_id": f"C-{p:02d}",
                "campaign_profile": p,
                "transform_chain": [TRANSFORMS[p % 6]],
                "timing_profile": TIMINGS[p % 4],
                "client_impl": CLIENTS[p % 6],
                "payload_size_class": SIZES[p % 4],
            }
...
def challenge_stage(args, manifest, events_out):
    ...
    for s, is_suspicious, kind in groups:
        ...
        for rep, suspicious in enumerate(variants):
            ...
            config = {
                "experiment_stage": "F_challenge",
                "challenge_kind": kind,
                "configuration_id": f"F-{profile_idx:03d}",
                "transform_chain": [TRANSFORMS[(profile_idx + rep) % 6]],
                "timing_profile": TIMINGS[(profile_idx + rep) % 4],
                "client_impl": "browser_chromium" if kind == "browser_native" else CLIENTS[(profile_idx + rep) % 6],
                "payload_size_class": SIZES[(profile_idx + rep) % 4],
                "open_set": kind in {"privacy_benign", "hard_negative"},
            }
...
def lots_stage(args, manifest, events_out):
    target = [
        ...
        if s.family in {"lots", "mqtt_ws", "doh"}
        or s.scenario_id in {"CC_BROWSER_09", "CC_BROWSER_10", "CC_BROWSER_11"}
    ]
    profile_plan = []
    for sid_prefix, count in [
        ("lots_chatops", 30), ("lots_tunnel", 20), ("bucket", 15),
        ("mqtt", 15), ("browser_exfil", 15), ("doh", 15),
    ]:
        for i in range(count):
            if sid_prefix == "lots_chatops":
                candidates = [s for s in target if s.scenario_id in {"CC_LOTS_01", "CC_LOTS_02", "CC_LOTS_03", "CC_LOTS_04"}]
            elif sid_prefix == "lots_tunnel":
                candidates = [s for s in target if s.scenario_id in {"CC_LOTS_05", "CC_LOTS_06", "CC_LOTS_07"}]
            elif sid_prefix == "bucket":
                candidates = [s for s in target if s.scenario_id == "CC_LOTS_08"]
            elif sid_prefix == "mqtt":
                candidates = [s for s in target if s.family == "mqtt_ws"]
            elif sid_prefix == "browser_exfil":
                candidates = [s for s in target if s.scenario_id.startswith("CC_BROWSER_")]
            else:
                candidates = [s for s in target if s.family == "doh"]
            profile_plan.append((candidates[i % len(candidates)], sid_prefix))

    for p, (s, kind) in enumerate(profile_plan):
        if p % args.shards != args.shard:
            continue
        for rep in range(10):
            ...
            for v in range(1 + benign_ratio):
                ...
                config = {
                    "experiment_stage": "G_commodity",
                    "profile_kind": kind,
                    "configuration_id": f"G-{p:03d}",
                    "transform_chain": [TRANSFORMS[(p + rep) % 6]],
                    "timing_profile": TIMINGS[(p + rep) % 4],
                    "client_impl": CLIENTS[(p + rep) % 6],
                    "payload_size_class": SIZES[(p + rep) % 4],
                }
...
def future_stage(args, manifest, events_out):
    """Transport-future holdout: real H3/QUIC, H3 datagrams, WebTransport,
    bounded CONNECT and explicit OHTTP binary privacy fixtures.

    20 H3 profiles and 20 tunnel profiles each produce 10 suspicious plus 20
    paired benign runs. 20 privacy profiles produce 10 benign-only runs.
    Total = 1,400 challenge sessions across all shards.
    """
    h3 = [s for s in SCENARIOS if s.family == "http3"]
    tunnels = [s for s in SCENARIOS if s.family in {"connect", "masque", "webtransport"}]
    privacy = [s for s in SCENARIOS if s.family == "privacy"]
    profiles = []
    profiles += [(h3[i % len(h3)], "h3_quic", True) for i in range(20)]
    profiles += [(tunnels[i % len(tunnels)], "future_tunnel", True) for i in range(20)]
    profiles += [(privacy[i % len(privacy)], "privacy_binary", False) for i in range(20)]

    for p, (s, kind, positive_profile) in enumerate(profiles):
        if p % args.shards != args.shard:
            continue
        variants = [True] * 10 + [False] * 20 if positive_profile else [False] * 10
        for rep, suspicious in enumerate(variants):
            persona, ip = PERSONAS[(p + rep) % len(PERSONAS)]
            cid = f"h-{p:03d}-{rep:02d}"
            configured = {
                "http3": "aioquic",
                "masque": "aioquic",
            ...
            config = {
                "experiment_stage": "F_future_challenge",
                "challenge_kind": kind,
                "configuration_id": f"H-{p:03d}",
                "transform_chain": [TRANSFORMS[(p + rep) % 6]],
                "timing_profile": TIMINGS[(p + rep) % 4],
                "client_impl": configured,
                "payload_size_class": SIZES[(p + rep) % 4],
                "open_set": True,
            }
            invoke(s.scenario_id, suspicious, args.seed + 70_000_000 + p * 1000 + rep, cid, f"run-{rep:02d}", persona, ip, 4, manifest, events_out, args.capture_file, config)


def mixed_stage(args, manifest, events_out):
    """Source-plan Stage D: 30 captures, each 60/90/120 actual minutes."""
    idx = args.mixed_index
    if idx is None:
        raise ValueError("--mixed-index is required for mixed stage")
    ...
    if suspicious_total == 1:
        suspicious_points = {flow_count // 2}
    ...
    for pos, i in enumerate(own):
        ...
        config = {
            ...
            "mixed_capture_index": idx,
            "logical_capture_minutes": duration,
            "configuration_id": f"D-{idx:02d}",
            "transform_chain": [TRANSFORMS[(i + idx) % 6]],
            "timing_profile": TIMINGS[(i + idx) % 4],
            "client_impl": client,
            "payload_size_class": SIZES[(i + idx) % 4],
            "mixed_prevalence": suspicious_total / max(1, flow_count),
        }
...
def main():
    ...
    p.add_argument("--stage", choices=["parser", "isolated", "sequence", "challenge", "lots", "future", "mixed"], required=True)
    ...
    if args.persona_index is not None:
        os.environ["COVERLAB_PERSONA_INDEX"] = str(args.persona_index)
