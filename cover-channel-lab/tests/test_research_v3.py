...
from coverlab.model_acceptance_v3 import adversarial_ok, environment_metrics_ok
...
def test_environment_metric_gate_requires_every_required_domain_cell():
    good={"status":"ok","rows":200,"positives":100,"negatives":100,"precision":1.0,"recall":0.99,"fpr":0.0}
    environment={
        "client_min_sessions":{"chromium":200},
        "server_min_sessions":{"nginx":100},
        "network_min_sessions":{"nat":200},
        "model_metrics":{"cells":{
            "client:chromium":dict(good),
            "server:nginx":dict(good),
            "network:nat":dict(good),
        }},
    }
    ready,checks,missing=environment_metrics_ok(environment,.95,.95,50/1_000_000)
    assert ready and not missing and all(x["passed"] for x in checks)
    del environment["model_metrics"]["cells"]["network:nat"]
    ready,_,missing=environment_metrics_ok(environment,.95,.95,50/1_000_000)
    assert not ready and missing==["network:nat"]
