...
# Real wire smoke for the new benign layer under actual netem.
...
MODEL="$WORK/model-fixture"; OUT="$WORK/models"; EVAL="$WORK/evaluation"
...
PYTHONPATH="$ROOT/src" python -m coverlab.ech_v3 --out "$EVAL/ech_capability.json"
PYTHONPATH="$ROOT/src" python -m coverlab.model_acceptance_v3 \
  ...
  --unseen-report "$EVAL/unseen.json" --out "$EVAL/model_acceptance_v3.json"
...
jq -e '.policy_revision == 3' "$EVAL/model_acceptance_v3.json" >/dev/null
