...
# usage: netem_v3.sh apply PROFILE [IFACE] | clear [IFACE]
...
case "$PROFILE" in
  ...
  constrained) sudo tc qdisc add dev "$IFACE" root handle 1: netem delay 60ms 10ms; sudo tc qdisc add dev "$IFACE" parent 1:1 handle 10: tbf rate 5mbit burst 64kb latency 400ms ;;
  nat_proxy) sudo tc qdisc add dev "$IFACE" root netem delay 25ms 4ms ;;
  inspected) sudo tc qdisc add dev "$IFACE" root netem delay 15ms 2ms ;;
  partial_capture) sudo tc qdisc add dev "$IFACE" root netem delay 25ms 5ms ;;
