...
sudo apt-get install -y software-properties-common tcpdump zstd jq curl ca-certificates openssl mosquitto mosquitto-clients iproute2 dnsutils openjdk-21-jdk-headless
...
for attempt in 1 2 3; do
  if sudo apt-get update -y && sudo apt-get install -y suricata; then
    suricata_ok=true
    break
  fi
  echo "Suricata install attempt $attempt failed; retrying" >&2
  sleep $((attempt * 10))
done
[[ "$suricata_ok" == true ]] || { echo "Suricata installation failed after retries" >&2; exit 1; }
...
javac -d /tmp/coverlab-java-client \
  "$ROOT/clients/CoverlabJavaClient.java" \
  "$ROOT/clients/StageMJavaWsClient.java" \
  "$ROOT/clients/StageMJavaTcpClient.java"
