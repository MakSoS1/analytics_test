...
def run(root:Path)->dict:
    built=0;skipped=0;errors=[];rows=0
    for pcap_zst in root.rglob('*.pcap.zst'):
        try:
            # release/bronze/SHARD/captures/SHARD.pcap.zst
            shard=pcap_zst.parent.parent.name
            bronze=pcap_zst.parent.parent.parent
            if bronze.name!='bronze':continue
            release=bronze.parent
            campaigns=bronze/shard/'manifests'/'campaigns.jsonl'
            target=release/'gold'/shard/'packet_sequence_features.parquet'
            ...
            with tempfile.TemporaryDirectory(prefix='coverlab-packet-backfill-') as td:
                td=Path(td);pcap=td/f'{shard}.pcap';stage=td/'stage';stage.mkdir();shutil.copy2(campaigns,stage/'campaigns.jsonl');decompress(pcap_zst,pcap)
                frame=_packet_sequence_features(stage,pcap)
