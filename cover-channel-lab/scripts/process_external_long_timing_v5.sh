...
EVIDENCE="$(realpath "$1")"
OUT="$(realpath -m "$2")"
MANIFEST="$EVIDENCE/long_timing_evidence.jsonl"
[[ -s "$MANIFEST" ]] || { echo "missing long_timing_evidence.jsonl" >&2; exit 1; }
mkdir -p "$OUT"

PYTHONPATH="$ROOT/src" python -m coverlab.external_campaign_adapter_v5 --kind long --manifest "$MANIFEST" --evidence-root "$EVIDENCE" --out-root "$OUT"
